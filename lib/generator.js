...
## 直接按顺序做
1. 双击 \`1-双击-安装依赖.bat\`
2. 打开 \`.env\`，把里面的空白值改成你自己的真实值
3. 打开 \`${projectConfig.accounts.file}\`，按示例格式填账号
4. 双击 \`2-双击-启动脚本.bat\`
...
if errorlevel 1 (
  ...
  echo Check your network or npm mirror, then try again.
  pause
  exit /b 1
)
...
echo.
echo Done.
echo Next: open .env and data files, then run the start helper bat file in this folder.
pause
`;
}
...
if not exist "node_modules" (
  echo Dependencies were not installed yet.
  echo Run the install helper bat file in this folder first.
  pause
  exit /b 1
)
...
call npm start
echo.
echo Project finished. Review the messages above.
pause
`;
}
...
## 使用步骤
1. 首次运行，双击 \`1-双击-安装依赖.bat\`
2. 打开 \`.env\`，填写需要的环境变量
3. 填写账号文件和代理文件
4. 必要时编辑 \`project.config.json\`，把示例 URL、合约地址、ABI、字段路径改成真实值
5. 双击 \`2-双击-启动脚本.bat\` 或手动运行 \`npm start\`
