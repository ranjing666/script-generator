  ...
  return entries
    .map((entry, index) => {
      ...
      const mimeType =
        ...
        "";
      const responseText =
        response.content && typeof response.content.text === "string"
          ? response.content.text
          : "";
      const parsedResponse =
        mimeType.includes("json") || responseText.trim().startsWith("{")
...
function findMessagePath(responseBody) {
  ...
  const messageMatch = rows.find(({ path, value }) => {
    const lowerPath = path.toLowerCase();
    return /(message|nonce|sign)/.test(lowerPath) && typeof value === "string";
  });

  return messageMatch ? messageMatch.path : "data.message";
}
...
function inferAuthStrategy(candidates, accountSource) {
  ...
  if (accountSource === "privateKeys" && nonceCandidates.length > 0 && authCandidates.length > 0) {
    return "evm_sign";
  }
...
function buildImportedAuth({ authMode, loginCandidate, nonceCandidate, accountSource, accountFields }) {
  ...
  if (authMode === "evm_sign") {
    return {
      type: "evm_sign",
      notes: ["该签名登录从抓包导入，请重点检查 messagePath 和 tokenPath。"],
      nonceRequest: {
      ...
      messagePath: findMessagePath(nonceCandidate.responseBody),
      loginRequest: {
        method: loginCandidate.method,
        url: applyReplacements(loginCandidate.url, replacements),
        headers: applyReplacements(loginCandidate.headers, replacements),
...
function scoreClaimMatch(listCandidate, claimCandidate, collectionInfo) {
  ...
  if (!listUrl.host || listUrl.host !== claimUrl.host) {
    return -1;
  }
