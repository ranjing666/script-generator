...
function parseJsonFile(filePath, sourceLabel) {
  const fullPath = path.resolve(filePath);
  let text = "";
  try {
    text = fs.readFileSync(fullPath, "utf8");
  } catch (error) {
    throw new Error(`${sourceLabel} 文件读取失败: ${error.message || String(error)}`);
  }

  const parsed = safeJsonParse(text);
  if (parsed === null) {
    throw new Error(`${sourceLabel} 文件不是有效 JSON，请重新导出后再试。`);
  }
...
function parseHarFile(filePath) {
  const har = parseJsonFile(filePath, "HAR");
  const entries = har && har.log && Array.isArray(har.log.entries) ? har.log.entries : null;
  if (!entries) {
    throw new Error("HAR 文件结构不正确：缺少 log.entries。请使用 “Save all as HAR with content” 重新导出。");
  }
...
function parsePostmanFile(filePath) {
  const collection = parseJsonFile(filePath, "Postman Collection");
  if (!collection || !Array.isArray(collection.item)) {
    throw new Error("Postman 文件结构不正确：缺少 item 列表。请导出 Collection v2.1 JSON。");
  }
  const flattenedItems = flattenPostmanItems(collection.item);
