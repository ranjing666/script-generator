...
function inferSiweMessageTemplate({ loginCandidate, nonceCandidate, noncePath }) {
  ...
  let template = messageRow.value;

  if (/0x[a-fA-F0-9]{40}/.test(template)) {
    template = template.replace(/0x[a-fA-F0-9]{40}/, "{{account.address}}");
  }

  const nonceValue =
    nonceCandidate && noncePath ? findValueByPath(nonceCandidate.responseBody, noncePath) : undefined;
  if (nonceValue !== undefined && nonceValue !== null && String(nonceValue).trim()) {
    template = template.split(String(nonceValue)).join("{{auth.nonce}}");
  }

  if (/\nNonce:\s*/i.test(template)) {
    template = template.replace(/(\nNonce:\s*)(.+)/i, "$1{{auth.nonce}}");
  }

  if (/\nIssued At:\s*/i.test(template)) {
    template = template.replace(/(\nIssued At:\s*)(.+)/i, "$1{{now.iso}}");
  }

  if (/\nExpiration Time:\s*/i.test(template)) {
    template = template.replace(/(\nExpiration Time:\s*)(.+)/i, "$1{{now.isoPlus10m}}");
  }
...
function buildImportedAuth({ authMode, loginCandidate, nonceCandidate, accountSource, accountFields }) {
  ...
  if (authMode === "evm_sign") {
    ...
    const siweMessageTemplate = inferSiweMessageTemplate({
      loginCandidate,
      nonceCandidate,
      noncePath,
    });
    ...
    return {
      ...
      ...(noncePath ? { noncePath } : {}),
      ...(siweMessageTemplate ? { messageTemplate: siweMessageTemplate } : { messagePath }),
      loginRequest: {
        method: loginCandidate.method,
        url: applyReplacements(loginCandidate.url, replacements),
        headers: applyReplacements(loginCandidate.headers, replacements),
