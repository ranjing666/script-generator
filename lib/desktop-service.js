...
const { createProject, buildProjectConfig } = require("./generator");
const {
...
function buildExecutionOptions(options, projectName) {
...
function resolveManualPlan(options) {
  ...
  const {
    ...
    concurrency,
    outputDir,
  } = buildExecutionOptions(options, projectName);
  ...
  return {
    ...projectOptions,
    projectConfig: buildProjectConfig(projectOptions),
  };
}

function previewManualProject(options) {
  const plan = resolveManualPlan(options);

  return {
    mode: "manual",
    outputDir: plan.outputDir,
    presetIds: plan.selectedPresets.map((preset) => preset.id),
...
function generateManualProject(options) {
  const plan = resolveManualPlan(options);
  const result = createProject(plan);
...
function generateImportProject(options) {
  const plan = resolveImportPlan(options);
  const result = createProject(plan.projectOptions);

  return {
    mode: "import",
    outputDir: result.outputDir,
    files: result.files,
    report: plan.importReport,
  };
}

function resolveImportPlan(options) {
...
function previewImportProject(options) {
  const plan = resolveImportPlan(options);

  return {
    mode: "import",
    outputDir: plan.projectOptions.outputDir,
    report: plan.importReport,
    projectConfig: plan.projectConfig,
  };
}
