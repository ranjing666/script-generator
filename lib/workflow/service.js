...
function runWorkflow(rootDir, options = {}) {
  const result = startRun(rootDir, options);

  if (options.workflow && options.projectId) {
    ...
    workflow.project.lastOutputDir = result.outputDir;
    workflow.artifacts.lastRunLogPath = result.logPath;
    saveProject(rootDir, workflow);
  }
...
async function pauseWorkflow(rootDir, projectId, workflow) {
  ...
  if (workflow) {
    ...
    nextWorkflow.runtime.run.status = result.status;
    nextWorkflow.runtime.run.lastRunId = result.runId;
    nextWorkflow.artifacts.lastRunLogPath = result.logPath;
    saveProject(rootDir, nextWorkflow);
  }
  return result;
}
