...
- 新增 `模板库`：预置“账号密码签到 / Token 保活 / EVM 日常任务 / 列表奖励领取 / 批量任务提交 / 链上综合日常”
- 新增 `模板画廊`：像工作流模板库一样，一眼看懂每种模板适合什么场景
- 新增 `模板搜索/标签筛选`：先按关键字和场景标签缩小范围，再选模板
- 新增 `一键导入`：自动识别抓包类型 + 自动分析 + 自动生成
- 新增 `导入可信度`：用分数和红黄绿提示告诉你这份抓包适不适合直接生成
- 新增 `导入支持范围`：直接告诉你这份抓包哪里完整、哪里可能还要手改
- 新增 `导入预览与系统建议`：分析后直接看到识别出的请求和下一步建议
- 新增 `候选请求复制代码`：每条候选请求可一键复制为 cURL / Node Axios
- 新增 `诊断助手`：分析失败、生成失败、警告过多时，直接给出修复方向
- 新增 `最近导入记录`：常用抓包文件可一键回填继续做
- 生成成功后自动附带 `.env`、`00-先看这里-零基础说明.md`、运行前检查脚本和双击启动脚本
...
在桌面版里，改完配置后还能先预览即将生成的 `project.config.json`，确认没问题再落盘。
...
- 自动把原始请求分组成“任务组”
- 自动给出推荐执行顺序
- 支持先预览最终 `project.config.json`，避免生成后才发现登录方式或任务顺序不对
...
现在 `2-双击-启动脚本.bat` 也会自动做两件事：
- 如果没装依赖，会先自动安装
- 启动前会先跑一次 `doctor.js`，把缺失项写进 `运行前检查报告.txt`
